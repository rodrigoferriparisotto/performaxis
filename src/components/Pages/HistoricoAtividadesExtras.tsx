...
import { registroDiurnasService, usuarioService, atividadeService } from '../../services/supabaseService';
...
const HistoricoAtividadesExtras: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    setLoading(true);
    try {
      const [registrosData, usuariosData, atividadesData, tiposData, count] = await Promise.all([
        registroDiurnasService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        carregarTiposExtras(),
        registroDiurnasService.getRegistrosCount()
      ]);
  ...
  const getTipoExtraNome = (tipoId: string) => {
    if (!tipoId) return 'Tipo não especificado';
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Histórico - Atividades Extras</h1>
        <p className="text-gray-600">Consulte os registros de atividades extras</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Registros de Atividades Extras</h2>
        ...
        <div className="p-6">
          {registros.length > 0 ? (
                ...
                <div key={registro.id} className="border border-gray-200 rounded-lg">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col lg:flex-row lg:items-center space-y-2 lg:space-y-0 lg:space-x-4 min-w-0 flex-1">
                        ...
                        {registro.tipo_atividade_id && (
                          <div className="flex items-center space-x-2 min-w-0">
                            <Tag className="w-4 h-4 text-gray-500" />
                            <span className="text-gray-600 text-sm lg:text-base truncate">{getTipoExtraNome(registro.tipo_atividade_id)}</span>
                          </div>
                        )}
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                                registroDiurnasService.deleteRegistro(registro.id).then(success => {
                                  if (success) {
                                    carregarDados();
                  ...
                  {registroSelecionado === registro.id && (
                    <div className="border-t border-gray-200 p-4 space-y-4">
                        ...
                        {registro.tipo_atividade_id && (
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {getTipoExtraNome(registro.tipo_atividade_id)}
                            </p>
                          </div>
                        )}
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-3">Atividades Extras</h4>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-gray-600">Os registros de atividades extras aparecerão aqui após serem criados</p>
            </div>
          )}
        </div>
...
export default HistoricoAtividadesExtras;