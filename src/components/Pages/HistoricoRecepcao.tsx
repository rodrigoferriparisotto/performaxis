...
import { registroRecepcaoService, usuarioService, atividadeService } from '../../services/supabaseService';
...
const HistoricoRecepcao: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      const [registrosData, usuariosData, atividadesData, count] = await Promise.all([
        registroRecepcaoService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        registroRecepcaoService.getRegistrosCount()
      ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Histórico - Recepção</h1>
        <p className="text-gray-600">Consulte os registros diários da recepção</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Registros Diários</h2>
        ...
        <div className="p-6">
          {registros.length > 0 ? (
                         ...
                         <button
                           onClick={(e) => {
                             ...
                             if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                               registroRecepcaoService.deleteRegistro(registro.id).then(success => {
                                 if (success) {
                                   carregarDados();
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-gray-600">Os registros diários aparecerão aqui após serem criados</p>
            </div>
          )}
        </div>
...
export default HistoricoRecepcao;