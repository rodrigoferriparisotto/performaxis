...
import { registroCamarariaService, usuarioService, atividadeService, suiteService } from '../../services/supabaseService';
...
const HistoricoCamararia: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      const [registrosData, usuariosData, atividadesData, suitesData, count] = await Promise.all([
        registroCamarariaService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        suiteService.getSuites(),
        registroCamarariaService.getRegistrosCount()
      ]);
  ...
  const getUsuarioProgramador = (registro: any) => {
    const foiProgramado = registro.observacoes?.includes('ORIGEM:PROGRAMADO');
    if (foiProgramado) {
      return getUsuarioNome(registro.usuario_id);
    }
    return null;
  };

  const getUsuarioExecutor = (registro: any) => {
    const foiProgramado = registro.observacoes?.includes('ORIGEM:PROGRAMADO');
    if (foiProgramado && registro.usuario_executor_id) {
      return getUsuarioNome(registro.usuario_executor_id);
    }
    return null;
  };
  ...
  const getTipoServicoColor = (tipo: string) => {
    switch (tipo) {
      case 'suite_livre': return 'bg-blue-100 text-blue-800';
      case 'permanencia': return 'bg-yellow-100 text-yellow-800';
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Histórico - Camararia</h1>
        <p className="text-gray-600">Consulte os registros de limpeza das suítes</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Registros de Limpeza</h2>
        ...
        <div className="p-6">
          {registros.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col lg:flex-row lg:items-center space-y-2 lg:space-y-0 lg:space-x-4 min-w-0 flex-1">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getTipoServicoColor(registro.tipo_servico)} hidden lg:inline-block`}>
                        ...
                        {getUsuarioProgramador(registro) ? (
                          <div className="flex flex-col space-y-1 min-w-0">
                            <div className="flex items-center space-x-2">
                              <User className="w-4 h-4 text-gray-500" />
                              <span className="text-gray-600 text-xs lg:text-sm truncate">
                                Prog: {getUsuarioProgramador(registro)}
                              </span>
                            </div>
                            {getUsuarioExecutor(registro) && (
                              <div className="flex items-center space-x-2">
                                <User className="w-4 h-4 text-blue-500" />
                                <span className="text-blue-600 text-xs lg:text-sm truncate font-medium">
                                  Exec: {getUsuarioExecutor(registro)}
                                </span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex items-center space-x-2 min-w-0">
                            <User className="w-4 h-4 text-gray-500" />
                            <span className="text-gray-600 text-sm lg:text-base truncate">
                              {getUsuarioNome(registro.usuario_executor_id || registro.usuario_id)}
                            </span>
                          </div>
                        )}
                         ...
                         <button
                           onClick={(e) => {
                             ...
                             if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                               registroCamarariaService.deleteRegistro(registro.id).then(success => {
                                 if (success) {
                                   carregarDados();
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{getTipoServicoLabel(registro.tipo_servico)}</p>
                          </div>
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-3">Atividades</h4>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-gray-600">Os registros de limpeza aparecerão aqui após serem criados</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {registros.reduce((acc, r) => 
                    acc + r.atividades.filter((a: any) => a.status === 'nao_realizada' || a.status === 'pendente').length, 0
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Suítes Atendidas</p>
                <p className="text-2xl font-bold text-gray-800">
...
export default HistoricoCamararia;