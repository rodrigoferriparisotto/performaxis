...
import { atividadeService, registroDiurnasService, cancelamentoService } from '../../services/supabaseService';
...
const NovoRegistroAtividadesExtras: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (registroAtivo && registroAtivo.atividades && registroAtivo.atividades.length > 0) {
      ...
      // PRIORIDADE 2: Para novos registros, filtrar por tipo E dia da semana
      const hoje = new Date().getDay();
      const diasSemana = ['domingo', 'segunda-feira', 'terça-feira', 'quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado'];
      const diaAtual = diasSemana[hoje];

      const atividadesFiltradas = todasAtividades.filter(atividade =>
        atividade.tipos_extras &&
        atividade.tipos_extras.includes(tipoSelecionado) &&
  ...
  const carregarDados = async () => {
    ...
    setLoading(true);
    try {
      // Carregar atividades extras e tipos de extras
      const [atividadesExtras, tiposData] = await Promise.all([
        atividadeService.getAtividadesByType('diurno'),
        carregarTiposExtrasComEmpresa()
      ]);

      setTodasAtividades(atividadesExtras);
      setTiposExtras(tiposData);

      // Verificar se há registro em andamento
      const registroEmAndamento = await registroDiurnasService.getRegistroEmAndamento(user.id, getDataAtual());
      ...
      if (registroEmAndamento) {
        ...
        if (registroEmAndamento.tipo_atividade_id && tiposData.some(t => t.id === registroEmAndamento.tipo_atividade_id)) {
          ...
          const atividadeCompleta = atividadesExtras.find(a => a.id === primeiraAtividade.atividade_id);

          if (atividadeCompleta && atividadeCompleta.tipos_extras && atividadeCompleta.tipos_extras.length > 0) {
            const tipoInferido = atividadeCompleta.tipos_extras[0];
  ...
  const carregarTiposExtrasComEmpresa = async () => {
    try {
  ...
  const iniciarRegistro = async () => {
    if (!tipoSelecionado) {
      alert('Por favor, selecione o tipo de atividade extra');
      return;
    }

    if (atividades.length === 0) {
      alert('Não há atividades cadastradas para este tipo de extra no dia de hoje');
      return;
    }
    ...
    try {
      const resultado = await registroDiurnasService.saveRegistro(novoRegistro);
      if (resultado) {
        setRegistroAtivo(resultado);
        await activityMarkingService.registrarInicio('atividades_extras');
      } else {
  ...
  const toggleAtividade = async (atividadeId: string) => {
    ...
    try {
      await registroDiurnasService.updateRegistro(registroAtivo.id, {
        atividades: atividadesAtualizadas,
        observacoes
      });
      await activityMarkingService.registrarAtividadeMarcada('atividades_extras');
    } catch (error) {
    }
  };
  ...
  const encerrarRegistro = async () => {
    ...
    try {
      const resultado = await registroDiurnasService.updateRegistro(registroAtivo.id, dadosFinalizacao);
      if (resultado) {
    ...
    if (confirm('Tem certeza que deseja cancelar? Todos os dados serão perdidos.')) {
      ...
      const cancelarAsync = async () => {
        try {
          ...
          // Excluir o registro
          const sucesso = await registroDiurnasService.deleteRegistro(registroAtivo.id);
          if (sucesso) {
            setRegistroAtivo(null);
            setObservacoes('');
  ...
  const getTipoExtraNome = (tipoId: string) => {
    const tipo = tiposExtras.find(t => t.id === tipoId);
  ...
  return (
    <div className="space-y-4 relative min-h-screen">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium text-gray-800">Atividades Extras</span>
                </div>
              </div>
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Novo Registro - Atividades Extras</h1>
        <p className="text-gray-600">Registre as atividades extras</p>
      </div>
        ...
        {!registroAtivo && (
          <div className="mt-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              Selecione o tipo de atividade extra *
            </label>
            {tiposExtras.length > 0 ? (
              <div className="relative">
                <select
                  ...
                  required
                >
                  <option value="">Selecione o tipo de atividade extra</option>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-yellow-800 text-sm">
                  <strong>Nenhum tipo de extra cadastrado.</strong>
                </p>
                <p className="text-yellow-700 text-xs mt-1">
                  Cadastre tipos de extras primeiro em Cadastros → Tipos de Extras
                </p>
              </div>
            )}
            ...
            {tipoSelecionado && (
              <p className="text-sm text-gray-600 mt-2">
                {atividades.length} atividades disponíveis para {getTipoExtraNome(tipoSelecionado)} hoje
              </p>
            )}
          </div>
        )}
          ...
          <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium text-yellow-800">
                Tipo: {getTipoExtraNome(registroAtivo.tipo_atividade_id || tipoSelecionado)}
              </span>
            </div>
          </div>
        )}
        ...
        <>
          <div className="bg-white rounded-lg shadow-md border border-gray-200">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-800">Atividades Extras</h3>
              <p className="text-sm text-gray-600">
                {getTipoExtraNome(registroAtivo.tipo_atividade_id || tipoSelecionado)} - Clique nas atividades para marcar como realizadas
              </p>
            </div>
              ...
              {atividades.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-gray-600">
                    Não há atividades cadastradas para o tipo "{getTipoExtraNome(registroAtivo.tipo_atividade_id || tipoSelecionado)}" no dia de hoje
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Escreva observações sobre as atividades extras realizadas..."
                />
              </div>
...
export default NovoRegistroAtividadesExtras;