...
import { atividadeService, registroAtividadesDiariasService, cancelamentoService } from '../../services/supabaseService';
...
const NovoRegistroAtividadesDiarias: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const registroEmAndamento = await registroAtividadesDiariasService.getRegistroEmAndamento(user.id);
      
      // Carregar atividades diárias e tipos de atividades da empresa
      const atividadesDiarias = await atividadeService.getAtividadesByType('noturno');
      ...
      setTodasAtividades(atividadesDiarias);
      ...
      if (registroEmAndamento) {
        ...
        if (registroEmAndamento.atividades && registroEmAndamento.atividades.length > 0) {
          ...
          const atividadeCompleta = atividadesDiarias.find(a => a.id === primeiraAtividade.atividade_id);
  ...
  const iniciarRegistro = async () => {
    ...
    try {
      const resultado = await registroAtividadesDiariasService.saveRegistro(novoRegistro);
      if (resultado) {
        setRegistroAtivo(resultado);
        await activityMarkingService.registrarInicio('atividades_diarias');
      } else {
  ...
  const toggleAtividade = async (atividadeId: string) => {
    ...
    try {
      await registroAtividadesDiariasService.updateRegistro(registroAtivo.id, {
        atividades: atividadesAtualizadas,
        observacoes
      });
      await activityMarkingService.registrarAtividadeMarcada('atividades_diarias');
    } catch (error) {
    }
  };
  ...
  const encerrarRegistro = async () => {
    ...
    try {
      const resultado = await registroAtividadesDiariasService.updateRegistro(registroAtivo.id, dadosFinalizacao);
      if (resultado) {
      ...
      const cancelarAsync = async () => {
        try {
          ...
          const cancelamentoData = {
            tipo: 'registro_atividades_diarias',
            usuario_id: user?.id || '',
          ...
          // Excluir o registro
          const sucesso = await registroAtividadesDiariasService.deleteRegistro(registroAtivo.id);
          if (sucesso) {
            setRegistroAtivo(null);
            setObservacoes('');
...
export default NovoRegistroAtividadesDiarias;