...
import { registroGestaoService, usuarioService, atividadeService } from '../../services/supabaseService';
...
const HistoricoGestao: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      const [registrosData, usuariosData, atividadesData, count] = await Promise.all([
        registroGestaoService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        registroGestaoService.getRegistrosCount()
      ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Histórico - Gestão</h1>
        <p className="text-gray-600">Consulte os registros de atividades de gestão</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Registros de Gestão</h2>
        ...
        <div className="p-6">
          {registros.length > 0 ? (
                         ...
                         <button
                           onClick={(e) => {
                             ...
                             if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                               registroGestaoService.deleteRegistro(registro.id).then(success => {
                                 if (success) {
                                   carregarDados();
                  ...
                  {registroSelecionado === registro.id && (
                    <div className="border-t border-gray-200 p-4 bg-gray-50">
                      <div className="space-y-6">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-3">Atividades de Gestão</h4>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-gray-600">Os registros de gestão aparecerão aqui após serem criados</p>
            </div>
          )}
        </div>
...
export default HistoricoGestao;