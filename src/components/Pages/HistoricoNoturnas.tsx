...
import { registroNoturnasService, usuarioService, atividadeService } from '../../services/supabaseService';
...
const HistoricoNoturnas: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      const [registrosData, usuariosData, atividadesData, count] = await Promise.all([
        registroNoturnasService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        registroNoturnasService.getRegistrosCount()
      ]);
                         ...
                         <button
                           onClick={(e) => {
                             ...
                             if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                               registroNoturnasService.deleteRegistro(registro.id).then(success => {
                                 if (success) {
                                   carregarDados();
...
export default HistoricoNoturnas;