...
import { registroCozinhaService, usuarioService, atividadeService } from '../../services/supabaseService';
...
const HistoricoCozinha: React.FC = () => {
  const { user } = useAuth();
  ...
  const carregarDados = async () => {
    ...
    try {
      const [registrosData, usuariosData, atividadesData, count] = await Promise.all([
        registroCozinhaService.getRegistros(currentPage, PAGE_SIZE),
        usuarioService.getUsuarios(),
        atividadeService.getAtividades(),
        registroCozinhaService.getRegistrosCount()
      ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Histórico - Cozinha</h1>
        <p className="text-gray-600">Consulte os registros diários da cozinha</p>
      </div>
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
                                registroCozinhaService.deleteRegistro(registro.id).then(success => {
                                  if (success) {
                                    carregarDados();
...
export default HistoricoCozinha;
