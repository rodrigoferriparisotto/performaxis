/*
  # Corrigir Limpeza de Histórico - Usar Campo 'data' ao invés de 'created_at'

  1. Problema Identificado
    - As funções de limpeza de histórico estavam usando o campo 'created_at' (data de criação no sistema)
    - Devem usar o campo 'data' (data real do evento/registro)
    - Isso impedia que registros retroativos fossem encontrados como "antigos"

  2. Correções Aplicadas
    - Atualiza função `consultar_registros_antigos` para usar campo 'data'
    - Atualiza função `executar_limpeza_historico` para usar campo 'data'
    - Mantém toda a lógica de permissões e auditoria existente

  3. Exemplo do Problema
    - Registro adicionado hoje com data de 8 meses atrás
    - created_at: hoje (recente) ❌
    - data: 8 meses atrás (antigo) ✅
    - Agora será corretamente identificado como antigo

  4. Notas Importantes
    - A correção preserva toda a segurança e auditoria
    - Apenas muda o critério de comparação de datas
    - Registros serão excluídos baseado na data real do evento
*/
...
BEGIN
  ...
  -- Manutenções (usando campo 'data_solicitacao')
  SELECT COUNT(*) INTO v_count
  FROM manutencoes
  WHERE empresa_id = p_empresa_id AND data_solicitacao::timestamptz < v_data_limite;
  v_resultado := v_resultado || jsonb_build_object(
    'tabela', 'manutencoes',
