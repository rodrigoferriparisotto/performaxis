/*
  # Fix column reference in obter_registros_pendentes function

  1. Changes
    - Fix references from s.nome to s.name in obter_registros_pendentes function
    - Affects registros_camararia and registros_revisao queries
    
  2. Reason
    - The suites table uses 'name' (English) while other tables use 'nome' (Portuguese)
    - This was causing a "column s.nome does not exist" error
*/

CREATE OR REPLACE FUNCTION public.obter_registros_pendentes(p_usuario_id uuid)
RETURNS TABLE(registro_id uuid, tipo_registro text, horas_decorridas numeric, info_adicional jsonb)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
